...
import idl.SmartDemo03.ReportMedia;
import idl.SmartDemo03.ReportMediaDataReader;
import idl.SmartDemo03.ReportMediaSeq;
import idl.SmartDemo03.ReportMediaTypeSupport;
...
public class ReportMediaDdsManager {
    ...
    // 用于存储接收到的图片数据
    private Map<String, MediaReceiver> mediaReceivers = new HashMap<>();

    // 应用内部存储路径
    //private String savePath;

    public interface OnMediaReceivedListener {
        void onMediaReceived(String alertId, Bitmap bitmap, String deviceId, String deviceType);
    }
    ...
    private void processMediaChunk(ReportMedia media) {
        String reportId = media.reportId;

        // 如果是新的媒体流，创建新的接收器
        if (!mediaReceivers.containsKey(reportId)) {
            mediaReceivers.put(reportId, new MediaReceiver(media.total_size));
            Log.i(TAG, String.format("开始接收媒体流: alertId=%s, deviceId=%s, type=%s, size=%d bytes",
                    reportId, media.deviceId, media.reportType, media.total_size));
        }
        ...
        if (isComplete) {
            ...
            mediaReceivers.remove(reportId); // 清理资源
        }
    }
            ...
            if (bitmap != null) {
                ...
                if (mediaListener != null) {
                    ...
                    new android.os.Handler(android.os.Looper.getMainLooper()).post(new Runnable() {
                        @Override
                        public void run() {
                            mediaListener.onMediaReceived(media.reportId, finalBitmap, media.deviceId, media.reportType);
                        }
                    });
                }
            } else {
