...
public class PresenceTypeSupport extends TypeSupport {
    ...
    public int print_sample(Object _sample) {
        ...
        if (sample.source != null){
            System.out.println("sample.source:" + sample.source);
        }
        else{
            System.out.println("sample.source: null");
        }
        System.out.println("sample.inRange:" + sample.inRange);
        System.out.println("sample.distance_cm:" + sample.distance_cm);
    ...
    public int get_sizeI(Object _sample,long cdr, int offset) throws UnsupportedEncodingException {
        ...
        Presence sample = (Presence)_sample;
        offset += CDRSerializer.get_string_size(sample.source == null ? 0 : sample.source.getBytes().length, offset);
    ...
    public int serializeI(Object _sample ,long cdr) {
        ...
        if (!CDRSerializer.put_string(cdr, sample.source, sample.source == null ? 0 : sample.source.length())){
            System.out.println("serialize sample.source failed.");
            return -2;
        }
        ...
        if (!CDRSerializer.put_int(cdr, sample.distance_cm)){
            System.out.println("serialize sample.distance_cm failed.");
            return -2;
        }
    ...
    synchronized public int deserializeI(Object _sample, long cdr){
        Presence sample = (Presence) _sample;
        sample.source = CDRDeserializer.get_string(cdr);
        if(sample.source ==null){
            System.out.println("deserialize member sample.source failed.");
            return -3;
        }
        ...
        if (!CDRDeserializer.get_int_array(cdr, tmp_int_obj, 1)){
            System.out.println("deserialize sample.distance_cm failed.");
            return -2;
        }
        sample.distance_cm= tmp_int_obj[0];
    ...
    public TypeCode get_typecode(){
        ...
        s_typeCode = factory.create_struct_TC("SmartDemo02.Presence");
        if (s_typeCode == null){
        ...
        if (memberTc == null){
            System.out.println("Get Member source TypeCode failed.");
        ...
        if (memberTc == null){
            System.out.println("Get Member distance_cm TypeCode failed.");
            factory.delete_TC(s_typeCode);
            s_typeCode = null;
            return null;
        }
